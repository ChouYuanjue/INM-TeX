...
\[
\begin{array}{c|c}
\text{metric} & \text{closeness}\\
\text{norm} & \text{vector size}\\
\text{matrix norm} & \text{matrix size}\\
\text{induced norm} & \text{maximal amplification}\\
\text{Frobenius norm} & \text{entrywise Euclidean size}\\
\text{spectral norm} & \text{largest singular value}
\end{array}
\]
