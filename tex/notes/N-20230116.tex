...
A function is data \(f:X\to Y\), not only a formula.  Injectivity says no information is identified; surjectivity says every target value is reached.  Bijectivity says the function is an isomorphism of sets.
\section{Images, preimages, and logic}
Preimages preserve all Boolean operations:
\[
f^{-1}(A\cap B)=f^{-1}(A)\cap f^{-1}(B),\qquad f^{-1}(Y\setminus A)=X\setminus f^{-1}(A).
\]
...
\section{Functional equations as constraints on orbits}
...
\section{Functions as structure-preserving maps}
...
\section{Morphisms and structural information loss}
