...
\begin{definition}[Elliptic curve in Weierstrass form]
An elliptic curve over a field is a nonsingular cubic curve with a chosen base point.  In Weierstrass form it is written as
\[
  y^2=x^3+ax+b,
\]
...
\section{Nonsingularity and the discriminant}
...
\section{The group law as divisor arithmetic}
...
\section{Associativity and geometry}
...
\]
\section{Cryptographic meaning}
Elliptic-curve cryptography uses the difficulty of recovering \(n\) from \(Q=nP\).  The geometry supplies a finite abelian group over \(\mathbb F_q\), and arithmetic security depends on the discrete logarithm problem in that group.
\section{Elliptic curves as geometry with a group law}
