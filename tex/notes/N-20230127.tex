...
\[
  1+x+x^2+\cdots+x^k
\]
whenever \(n\) is not a multiple of \(k+1\).  This first guess is too weak.  The correct condition is coprimality.

\begin{theorem}[Root-of-unity test for the generalization]
...
\]
are exactly the nontrivial \((k+1)\)-st roots of unity.  Let \(\xi\ne1\) satisfy \(\xi^{k+1}=1\).  We need
\[
  1+\xi^n+\xi^{2n}+\cdots+\xi^{kn}=0
\]
for every such \(\xi\).

This is a geometric sum.  It vanishes exactly when \(\xi^n\) is a nontrivial \((k+1)\)-st root of unity.  Therefore the map
\[
  \xi\longmapsto \xi^n
\]
must send every nontrivial \((k+1)\)-st root to another nontrivial \((k+1)\)-st root.  Equivalently, the power map on the cyclic group \(\mu_{k+1}\) must have trivial kernel.  This happens exactly when \(n\) is invertible modulo \(k+1\), i.e.
\[
  \gcd(n,k+1)=1.
\]
\end{proof}

The original false guess \(n\not\equiv0\pmod{k+1}\) fails because a nonzero residue may still not be invertible.  For instance, modulo \(6\), the residue \(2\) is nonzero but the map \(z\mapsto z^2\) has a nontrivial kernel on sixth roots of unity.
...
\section{Cyclotomic summary of the experiment}

The computational pictures, the failed generalization, and the final divisibility criterion all express one group-theoretic fact: roots of unity form finite cyclic groups, and power maps on cyclic groups are controlled by greatest common divisors.

\begin{remark}[From roots to factors]
A rational polynomial divisibility statement can be checked on all Galois conjugate roots.  For roots of unity, the conjugates of a primitive \(m\)-th root are
\[
  \zeta_m\longmapsto \zeta_m^a,
  \qquad a\in(\mathbb Z/m\mathbb Z)^\times.
\]
Thus a common factor is not merely one common complex root; it is a conjugation-stable set of common roots.
\end{remark}
...
\[
  x^m-1=\prod_{d\mid m}\Phi_d(x)
\]
separates roots by exact order.  Geometric sums select all nontrivial roots of a given order divisor, while substitutions \(x\mapsto x^n\) pull those root conditions back along the power map.  This is the structural explanation of the root plots in the original experiment.
