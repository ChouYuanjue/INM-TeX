...
\begin{tikzpicture}[>=stealth,node distance=2.2cm]
  \node[draw,circle,minimum size=0.9cm] (z) {$z$};
  \node[draw,circle,minimum size=0.9cm,right of=z] (x) {$x$};
  \draw[->,thick] (z) -- node[above] {$p_\theta(x\mid z)$} (x);
  \draw[->,thick,bend left=35] (x) to node[below] {$q_\phi(z\mid x)$} (z);
  \node[below of=z,node distance=1.0cm] {latent};
  \node[below of=x,node distance=1.0cm] {observed};
\end{tikzpicture}
\end{center}
...
\section{What remains to derive}
