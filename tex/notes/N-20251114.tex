...
\begin{tikzpicture}[>=stealth,scale=0.72,transform shape]
  \node[draw,rounded corners,align=center,minimum width=2.6cm,minimum height=0.75cm] (T0) at (0,0) {Theater \(0\)};
  \node[draw,rounded corners,align=center,minimum width=2.6cm,minimum height=0.75cm] (T1) at (4.4,0) {Theater \(1\)};
  \node[draw,rounded corners,align=center,minimum width=2.7cm,minimum height=0.75cm] (R0) at (0,-1.7) {reconstructed\ ring data};
  \node[draw,rounded corners,align=center,minimum width=2.7cm,minimum height=0.75cm] (R1) at (4.4,-1.7) {reconstructed\ ring data};
  \node[draw,rounded corners,align=center,minimum width=3.0cm,minimum height=0.75cm] (S) at (2.2,-3.25) {log-shell /\ indeterminacy};
  \draw[->] (T0) -- node[above] {theta-link} (T1);
  \draw[->] (T0) -- node[left] {mono-anabelian} (R0);
  \draw[->] (T1) -- node[right] {mono-anabelian} (R1);
  \draw[->] (R0) -- node[below left] {log-link} (S);
  \draw[->] (R1) -- node[below right] {log-link} (S);
  \draw[dashed,->] (R0) -- node[above] {not ring iso} (R1);
\end{tikzpicture}
\end{center}
