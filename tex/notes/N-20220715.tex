...
For this chapter, the elementary material should be read as a study of what remains invariant when coordinates change. The earlier sections (`Conic sections`, `Latus rectum and tangent geometry`, `Rational and irrational density`, `Vector spaces`, `Dedekind cuts and order`) compute with arrays, bases, pivots, determinants, or eigenvectors; the deeper object is a linear map together with the spaces it acts on. A matrix is a representation of that map after bases have been chosen. This is why formulas such as
\[
...
\section{Pedagogical repair: conics and cuts as two kinds of construction}

The conic part and the Dedekind-cut part look unrelated, but both are about turning an intuitive geometric object into an exact algebraic definition.  An ellipse is not just a picture; it is the set of points satisfying a focus-distance equation.  A real number is not just a point on a drawn line; it can be encoded as a cut in \(\mathbb Q\).

This is a recurring mathematical move: replace a mental image by a definition that can be manipulated.  Once that is done, one can prove tangent formulas for conics and algebraic laws for real numbers.
